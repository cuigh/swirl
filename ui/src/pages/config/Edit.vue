    ...
    <n-form :model="model" :rules="rules" ref="form" label-placement="top" label-width="90">
      <n-grid cols="1 640:2" :x-gap="24">
        <n-form-item-gi :label="t('fields.name')" path="name">
          <n-input :placeholder="t('fields.name')" v-model:value="model.name" :disabled="Boolean(model.id)" />
        </n-form-item-gi>
...
async function fetchData() {
  ...
  if (id) {
    let tr = await configApi.find(id);
    model.value = tr.data?.config as Config;
  }
}
