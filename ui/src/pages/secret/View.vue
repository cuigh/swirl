    ...
    <n-tabs type="line" style="margin-top: -12px">
      <n-tab-pane name="detail" :tab="t('fields.detail')">
        <n-space vertical :size="16">
          ...
          <x-panel :title="t('fields.driver')" v-if="model.driver.name">
            <x-description label-align="left" :label-width="40">
              <x-description-item :label="t('fields.name')" :span="2">
                <n-tag round size="small" type="warning">{{ model.driver.name }}</n-tag>
              </x-description-item>
            </x-description>
            ...
            <n-table size="small" :bordered="true" :single-line="false">
              ...
              <tbody>
                <tr v-for="label in model.driver.options">
                  <td>{{ label.name }}</td>
                  <td>{{ label.value }}</td>
                </tr>
              </tbody>
...
const model = ref({ driver: {}, templating: {} } as Secret);
const raw = ref('');

async function fetchData() {
  const id = route.params.id as string;
  let r = await secretApi.find(id);
  model.value = r.data?.secret as Secret;
